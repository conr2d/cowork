...
- **Multi-session UI (v0.2+):** the eventual main window is a sidebar of user workspaces, each bound to its own live terminal session (a coding-agent TUI, not a web chat). Design the WP8 PTY bridge **session-keyed from day one**: a `SessionId` newtype as the canonical key across the host driver ↔ frontend, a host registry mapping `SessionId → session handle`, and per-session keyed output events (`session-output:{id}`). Prefer an **actor-per-session** model (one task owns the child process + I/O + protocol state, fed by a bounded channel) over a shared-locked process map. v0.1 ships exactly one session (the default `~/cowork/` workspace) but must **not** bake in a single-session assumption — retrofitting concurrent sessions later is the known failure mode of comparable tools. (Reference implementations exist in the Tauri/Claude-Code-GUI space; study before building v0.2, and never copy AGPL-licensed code — patterns only.)
